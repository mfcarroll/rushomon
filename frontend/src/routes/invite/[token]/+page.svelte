<script lang="ts">
	...
	$effect(() => {
		...
		orgsApi
			...
			.then((info) => {
				// If they are already a member (e.g. from JIT flow), bypass everything and go to dashboard
			...
			{:else}
				...
				{#if data.user}
					...
					{#if inviteInfo.is_member}
						<div class="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-5 text-sm text-blue-800 text-center">
