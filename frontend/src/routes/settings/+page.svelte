			...
			{#if data.user}
				...
				>
					<div class="flex justify-between items-center mb-4">
						<div>
							...
							<p class="text-xs text-gray-500 mt-1">
								Manage Personal Access Tokens for programmatic
								access to the API.
							</p>
						</div>
						{#if canCreateApiKeys()}
					...
					{:else if apiKeys.length === 0}
						<p
							class="text-sm text-gray-500 py-2 border-t border-gray-100"
						>
	...
	>
		<div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
			...
			{#if newlyGeneratedToken}
				...
				>
					...
					<p class="text-xs text-amber-700 mb-4">
						For your security, we only show this token once. If you
						lose it, you will need to revoke it and generate a new
						one.
					</p>
			...
			{:else}
				...
				{#if createError}
					<div
						class="bg-red-50 border border-red-200 rounded-xl p-4 mb-5"
					>
		...
		>
			...
			<div class="mb-6">
				<p class="text-gray-600 mb-3">
					...
					integrations using it will immediately break.
				</p>
