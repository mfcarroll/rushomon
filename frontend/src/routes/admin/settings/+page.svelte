<script lang="ts">
	...
	function getPricingInfoForSlot(
		...
		if (discount.type === "fixed") {
			discountedPrice = Math.max(
				0,
				basePrice - (discount.amount || 0) / 100,
			);
		} else {
			discountedPrice =
				basePrice * (1 - (discount.basis_points || 0) / 10000);
		}
	...
	async function loadSettings() {
		try {
			...
			discountSlots.pro_annual =
				settings.active_discount_pro_annual || "";
			...
			productSlots.business_annual =
				settings.product_business_annual_id || "";
		...
		try {
			...
			if (selectedDiscount) {
				if (selectedDiscount.type === "fixed") {
					...
					const basePrice =
						basePrices[slot as keyof typeof basePrices] || 0;
					const discountAmount = Math.round(
						(basePrice * (selectedDiscount.basis_points || 0)) /
							10000,
					);
	...
	function hasUnsavedChanges(): boolean {
		return (
			...
			productSlots.business_monthly !==
				initialProductSlots.business_monthly ||
	...
	function getSelectedProductPrice(slot: keyof typeof productSlots): string {
		...
		if (
			!productId ||
			productId === initialProductSlots[slot] ||
			!products
		) {
			return "NA";
		}
	...
	async function saveProductConfiguration() {
		try {
			...
			if (result.success) {
				...
				showToastMessage(
					"Product configuration saved and cached successfully",
				);
			} else {
				showToastMessage(
	...
	async function confirmFounderPricingToggle() {
		...
		try {
			...
			founderPricingEnabled =
				updatedSettings.founder_pricing_active === "true";
			showToastMessage(
								...
								<div class="product-pricing-info">
									...
									<div class="price-comparison">
										...
										<div class="price-item next">
											...
											<span
												class="price-value"
												class:na={getSelectedProductPrice(
													row.slot,
												) === "NA"}
									...
									<select
										id="product-{row.slot}"
										value={productSlots[
											row.slot as keyof typeof productSlots
										]}
										onchange={(e) => {
											const val = (
			...
			<div class="setting-card">
				<div class="setting-info" style="margin-bottom: 1rem;">
					...
					<p class="setting-description">
						...
						combination. Applied automatically at checkout when
						Founder Pricing is enabled.
					</p>
				</div>
				{#if discountsLoading}
					<p class="setting-description">
						...
						{#each [{ key: "active_discount_pro_monthly", label: "Pro", interval: "Monthly", slot: "pro_monthly" as const, basePrice: 9 }, { key: "active_discount_pro_annual", label: "Pro", interval: "Annual", slot: "pro_annual" as const, basePrice: 90 }, { key: "active_discount_business_monthly", label: "Business", interval: "Monthly", slot: "business_monthly" as const, basePrice: 29 }, { key: "active_discount_business_annual", label: "Business", interval: "Annual", slot: "business_annual" as const, basePrice: 290 }] as row}
							<div class="discount-row">
										...
										{#if discountSlots[row.slot]}
											{@const selectedDiscount =
												discounts.find(
													(d) =>
														d.id ===
														discountSlots[row.slot],
												)}
											{#if selectedDiscount}
												{@const pricing =
													getPricingInfoForSlot(
														row.slot,
														selectedDiscount,
													)}
												{#if pricing}
								...
								<select
									...
									value={discountSlots[
										row.slot as keyof typeof discountSlots
									]}
									onchange={(e) => {
										const val = (
											e.target as HTMLSelectElement
			...
			<div class="setting-card">
				<div class="setting-content">
					<div class="setting-info">
						...
						<p class="setting-description">
							Show founder pricing badges and apply active
