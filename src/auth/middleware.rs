...
use crate::utils::time::now_timestamp; // Assuming you have a time util, or use chrono
    ...
    let jwt = if let Some(token) = jwt {
        ...
        match req.headers().get("Cookie") {
            Ok(Some(header)) => match parse_cookie_header(&header) {
                Some(token) => token,
                None => return Err(AuthError::Unauthorized("Authentication required".to_string())),
            },
            Ok(None) => return Err(AuthError::Unauthorized("Authentication required".to_string())),
            Err(_e) => return Err(AuthError::InternalError("Failed to read headers".to_string())),
        }
    };
    ...
    if jwt.starts_with("ro_pat_") {
        let db = match ctx.env.get_binding::<D1Database>("rushomon") {
            ...
            Err(_e) => {
                ...
                return Err(AuthError::InternalError("Server configuration error".to_string()));
            }
        };
        ...
        let api_key = match crate::db::queries::get_api_key_by_hash(&db, &key_hash).await {
            ...
            Err(_) => return Err(AuthError::InternalError("Failed to validate API key".to_string())),
        };
        ...
        let user = match crate::db::get_user_by_id(&db, &api_key.user_id).await {
            ...
            Err(_) => return Err(AuthError::InternalError("Failed to validate user".to_string())),
        };
        ...
        let _ = crate::db::queries::update_api_key_last_used(&db, &api_key.id, now_timestamp()).await;
